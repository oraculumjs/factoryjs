...
Factory = require '../src/Factory.coffee'
...
describe "Factory", ->
  ...
  it 'should allow the baseTags property to be set in the constructor', ->
    ...
    factory = new Factory null, {baseTags}
  ...
  describe "factory instance", ->
    ...
    describe "define method", ->
      ...
      it "should emit an event", ->
        ...
        expect(emit).toHaveBeenCalledWith 'define', 'test', test
    ...
    describe "fetchDefinition method", ->
      ...
      it "should return a promise when called", ->
        expect(factory.fetchDefinition('underscore')).toBePromise()

      it "should resolve the promise when the definition gets retrieved", ->
        resolved = false
        waitsFor -> resolved
        expect(factory.hasDefinition('underscore')).toBe false
        promise = factory.fetchDefinition 'underscore'
        promise.done (f) ->
          resolved = true
          expect(factory.hasDefinition('underscore')).toBe true
    ...
    describe "defineMixin method", ->
      ...
      it 'should emit an event', ->
        expect(emit).toHaveBeenCalledOnce()
        expect(emit).toHaveBeenCalledWith 'defineMixin', 'test', mixin
